...
import type { ZakLinePoint, ZakLineData } from '@blue/data';
...
function toCanvas(p: ZakLinePoint, w: number): { cx: number; cy: number } {
...
function fromCanvas(cx: number, cy: number, w: number): ZakLinePoint {
  const plotW = w - PAD_L - PAD_R;
  const plotH = CANVAS_H - PAD_T - PAD_B;
  return {
...
interface LineCanvasProps {
  line: ZakLineData;
  width: number;
  onChange: (points: ZakLinePoint[]) => void;
}
...
export default function ZakLineObjectEditor({
  ...
  const handleAddLine = useCallback(() => {
    ...
    const defaultColors = [
      0x6699cc, 0xcc6699, 0x66cc99, 0xcc9966, 0x9966cc, 0x99cc66,
    ];
    const maxChannel = lines.reduce(
      (max, l) => Math.max(max, l.channel),
      0,
    );
    const newLine: ZakLineData = {
  ...
  const handleSplitMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      if (parent) {
        ...
        setSplitX(Math.max(140, Math.min(400, ev.clientX - rect.left)));
      }
    };
    const onUp = () => {
  ...
  return (
    <div className="flex h-full select-none">
      ...
      >
        ...
        {selectedLine && (
          <div className="border-t border-blue-border px-2 py-1.5 space-y-1.5">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleChannelChange(parseInt(e.target.value, 10) || 1)
                }
              />
            </div>
          </div>
        ...
        <div className="border-t border-blue-border px-2 py-1.5">
          ...
          <input
            ...
            value={zakSpace}
            onChange={(e) =>
              patch({ zakSpace: parseInt(e.target.value, 10) || 0 })
            }
          />
        </div>
      </div>
