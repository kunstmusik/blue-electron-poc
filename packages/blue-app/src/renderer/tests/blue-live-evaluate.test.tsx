...
const { resolveNamedInstrumentNumbers, normalizeScoreForEngineApi } = await import(
...
describe('Editor context menu enablement (T075)', () => {
  it('evaluate code keybinding module loads', async () => {
    const mod = await import('../components/workbench/panels/editors/csound-editor-evaluation');
    expect(mod.createEvaluateCodeKeyBindings).toBeDefined();
    expect(mod.getEvaluableCodeRange).toBeDefined();
    expect(mod.evaluateCodeFromEditor).toBeDefined();
  });

  it('createEvaluateCodeKeyBindings returns Cmd-Enter and Ctrl-Enter', async () => {
    const { createEvaluateCodeKeyBindings } = await import('../components/workbench/panels/editors/csound-editor-evaluation');
  ...
  it('createEvaluateCodeKeyBindings returns false when disabled', async () => {
    const { createEvaluateCodeKeyBindings } = await import('../components/workbench/panels/editors/csound-editor-evaluation');
...
describe('Cmd-Return shortcut tests (T076)', () => {
  it('keymap extension is created without error', async () => {
    const { createEvaluateCodeKeymapExtension } = await import('../components/workbench/panels/editors/csound-editor-evaluation');
    const ext = createEvaluateCodeKeymapExtension('orc', () => vi.fn(), () => true);
    expect(ext).toBeDefined();
  });

  it('keymap extension works for sco mode', async () => {
    const { createEvaluateCodeKeymapExtension } = await import('../components/workbench/panels/editors/csound-editor-evaluation');
