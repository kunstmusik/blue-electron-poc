...
- **FR-004**: General settings MUST include Work Directory, New User Defaults Enabled, Draw Alpha Background on Marquee, Message Colors Enabled, Csound Error Warning Enabled, and Max Temp Files per Directory, with Java defaults of empty work directory, true, false, false, true, and 3 respectively.
...
- **FR-024**: General settings MUST be consumed where Java Blue consumes them, including work-directory defaults for file choosers/import/export flows, New User Defaults behavior, alpha marquee drawing, message color behavior in render commands, Csound error warning behavior, and temporary-file-per-directory limit.
...
- **Alpha Marquee And Csound Error Warning**: Java Blue consumes these in UI selection marquee drawing and Csound output warning behavior. If equivalent workflows are missing or different, record the gap with a follow-up or an explicit non-parity decision.
